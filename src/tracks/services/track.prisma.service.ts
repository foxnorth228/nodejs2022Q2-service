...
export class TrackPrismaService extends elementPrismaService<ITrack> {
    ...
    async deleteArtistFromTracks(id: string) {
        ...
        for (const track of tracks) {
          if (track.artistId === id) {
            await this.update(track.id, { name: track.name, duration: track.duration, artistId: null, albumId: track.albumId})
            count++;
          }
        }
    ...
    async deleteAlbumFromTracks(id: string) {
        ...
        for (const track of tracks) {
          if (track.albumId === id) {
            await this.update(track.id, { name: track.name, duration: track.duration, artistId: track.artistId, albumId: null})
            count++;
          }
        }
